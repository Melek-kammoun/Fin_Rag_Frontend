...
export class AdminRegulationUploadComponent implements OnInit, OnDestroy {
  ...
  private watchJob(jobId: number): void {
    ...
    // Synchronisation initiale après un rechargement, puis mises à jour temps réel.
    this.api.getJob(jobId).subscribe({
      next: (job) => (this.currentJob = job),
      error: () => {
        /* Le WebSocket peut encore fournir les prochaines mises à jour. */
      },
    });
    ...
    this.jobSubscription = this.jobSocket.watchJob(jobId).subscribe({
      ...
      error: (error) => {
        this.errorMessage =
          error?.error?.message ||
          error?.message ||
          'Unable to receive job status updates.';
      },
    });
  }
